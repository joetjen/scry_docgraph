  ...
  (
    ...
    def lex_token__SPREAD(input0) do
      with {:ok, t0, rest0} <- lex_expr__0(input0),
           ...
           {:ok, t2, rest2} <- lex_expr__2(rest1) do
        {:ok, t0 <> t1 <> t2, rest2}
      else
        :fail -> :fail
      end
    ...
    def lex_token__LE(input0) do
      with {:ok, t0, rest0} <- lex_expr__3(input0), {:ok, t1, rest1} <- lex_expr__4(rest0) do
        {:ok, t0 <> t1, rest1}
      else
        :fail -> :fail
      end
    ...
    def lex_token__KW_ANCESTORS(input0) do
      with {:ok, t0, rest0} <- lex_expr__5(input0),
           {:ok, t1, rest1} <- lex_expr__6(rest0),
           {:ok, t2, rest2} <- lex_expr__7(rest1),
           {:ok, t3, rest3} <- lex_expr__8(rest2),
           {:ok, t4, rest4} <- lex_expr__9(rest3),
           {:ok, t5, rest5} <- lex_expr__10(rest4),
           {:ok, t6, rest6} <- lex_expr__11(rest5),
           {:ok, t7, rest7} <- lex_expr__12(rest6),
           {:ok, t8, rest8} <- lex_expr__13(rest7) do
        {:ok, t0 <> t1 <> t2 <> t3 <> t4 <> t5 <> t6 <> t7 <> t8, rest8}
      else
        :fail -> :fail
      end
    end
    ...
    def lex_expr__15(input) do
      Tokenizer.first_char_match([&lex_expr__16/1, &lex_expr__17/1], input)
    end
    ...
    def lex_expr__28(input) do
      case input do
        <<c::utf8, rest::binary>> ->
          if Tokenizer.in_ranges?(c, [{119, 119}, {87, 87}]) do
            {:ok, <<c::utf8>>, rest}
          else
            :fail
          end
    ...
    def lex_expr__31(input) do
      case input do
        <<c::utf8, rest::binary>> ->
          if Tokenizer.in_ranges?(c, [{100, 100}, {68, 68}]) do
            {:ok, <<c::utf8>>, rest}
          else
            :fail
          end
    ...
    def lex_token__KW_BACKWARD(input0) do
      with {:ok, t0, rest0} <- lex_expr__24(input0),
           ...
           {:ok, t7, rest7} <- lex_expr__31(rest6) do
    ...
    def lex_expr__39(input0) do
      with {:ok, t0, rest0} <- lex_expr__40(input0),
           {:ok, t1, rest1} <- lex_expr__41(rest0),
           {:ok, t2, rest2} <- lex_expr__42(rest1),
           {:ok, t3, rest3} <- lex_expr__43(rest2),
           {:ok, t4, rest4} <- lex_expr__44(rest3) do
    ...
    def lex_token__KW_PARENT(input0) do
      with {:ok, t0, rest0} <- lex_expr__47(input0),
           {:ok, t1, rest1} <- lex_expr__48(rest0),
           {:ok, t2, rest2} <- lex_expr__49(rest1),
           {:ok, t3, rest3} <- lex_expr__50(rest2),
           {:ok, t4, rest4} <- lex_expr__51(rest3),
           {:ok, t5, rest5} <- lex_expr__52(rest4) do
    ...
    def lex_expr__59(input) do
      Tokenizer.not_pred_char(&lex_expr__60/1, input)
    end
    ...
    def lex_token__STRING(input) do
      Tokenizer.first_char_match([&lex_expr__53/1, &lex_expr__63/1], input)
    end
    ...
    def lex_token__GE(input0) do
      with {:ok, t0, rest0} <- lex_expr__73(input0), {:ok, t1, rest1} <- lex_expr__74(rest0) do
        {:ok, t0 <> t1, rest1}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__80(input) do
      Tokenizer.first_char_match([&lex_expr__81/1, &lex_expr__82/1], input)
    end

    def lex_expr__79(input) do
      Tokenizer.star_char(&lex_expr__80/1, input)
    end
    ...
    def lex_token__ESCAPED_IDENT(input0) do
      with {:ok, t0, rest0} <- lex_expr__75(input0),
           {:ok, t1, rest1} <- lex_expr__76(rest0),
    ...
    def lex_expr__105(input) do
      Tokenizer.opt_char(&lex_expr__106/1, input)
    end
    ...
    def lex_expr__113(input0) do
      with {:ok, t0, rest0} <- lex_expr__114(input0),
           ...
           {:ok, t5, rest5} <- lex_expr__119(rest4) do
        {:ok, t0 <> t1 <> t2 <> t3 <> t4 <> t5, rest5}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__110(input) do
      Tokenizer.opt_char(&lex_expr__111/1, input)
    end

    def lex_expr__95(input0) do
      with {:ok, t0, rest0} <- lex_expr__96(input0),
           {:ok, t1, rest1} <- lex_expr__97(rest0),
           {:ok, t2, rest2} <- lex_expr__98(rest1),
           {:ok, t3, rest3} <- lex_expr__99(rest2),
           {:ok, t4, rest4} <- lex_expr__100(rest3),
           {:ok, t5, rest5} <- lex_expr__101(rest4),
           {:ok, t6, rest6} <- lex_expr__102(rest5),
           {:ok, t7, rest7} <- lex_expr__103(rest6),
           {:ok, t8, rest8} <- lex_expr__104(rest7),
           {:ok, t9, rest9} <- lex_expr__105(rest8),
           {:ok, t10, rest10} <- lex_expr__110(rest9) do
    ...
    def lex_token__DATE(input0) do
      with {:ok, t0, rest0} <- lex_expr__84(input0),
           ...
           {:ok, t2, rest2} <- lex_expr__86(rest1),
           {:ok, t3, rest3} <- lex_expr__87(rest2),
           {:ok, t4, rest4} <- lex_expr__88(rest3),
           {:ok, t5, rest5} <- lex_expr__89(rest4),
           {:ok, t6, rest6} <- lex_expr__90(rest5),
           {:ok, t7, rest7} <- lex_expr__91(rest6),
           {:ok, t8, rest8} <- lex_expr__92(rest7),
           {:ok, t9, rest9} <- lex_expr__93(rest8),
           {:ok, t10, rest10} <- lex_expr__94(rest9) do
        {:ok, t0 <> t1 <> t2 <> t3 <> t4 <> t5 <> t6 <> t7 <> t8 <> t9 <> t10, rest10}
      else
        :fail -> :fail
      end
    ...
    def lex_token__KW_SHORTEST(input0) do
      with {:ok, t0, rest0} <- lex_expr__120(input0),
           {:ok, t1, rest1} <- lex_expr__121(rest0),
           {:ok, t2, rest2} <- lex_expr__122(rest1),
           {:ok, t3, rest3} <- lex_expr__123(rest2),
           {:ok, t4, rest4} <- lex_expr__124(rest3),
           {:ok, t5, rest5} <- lex_expr__125(rest4),
           {:ok, t6, rest6} <- lex_expr__126(rest5),
           {:ok, t7, rest7} <- lex_expr__127(rest6) do
        {:ok, t0 <> t1 <> t2 <> t3 <> t4 <> t5 <> t6 <> t7, rest7}
      else
        :fail -> :fail
      end
    ...
    def lex_token__KW_PATH(input0) do
      with {:ok, t0, rest0} <- lex_expr__128(input0),
           {:ok, t1, rest1} <- lex_expr__129(rest0),
           {:ok, t2, rest2} <- lex_expr__130(rest1),
           {:ok, t3, rest3} <- lex_expr__131(rest2) do
        {:ok, t0 <> t1 <> t2 <> t3, rest3}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__140(input) do
      case input do
        <<c::utf8, rest::binary>> ->
          if Tokenizer.in_ranges?(c, [{108, 108}, {76, 76}]) do
            {:ok, <<c::utf8>>, rest}
          else
            :fail
          end
    ...
    def lex_token__KW_SIBLINGS(input0) do
      with {:ok, t0, rest0} <- lex_expr__137(input0),
           {:ok, t1, rest1} <- lex_expr__138(rest0),
           {:ok, t2, rest2} <- lex_expr__139(rest1),
           {:ok, t3, rest3} <- lex_expr__140(rest2),
           {:ok, t4, rest4} <- lex_expr__141(rest3),
           {:ok, t5, rest5} <- lex_expr__142(rest4),
           {:ok, t6, rest6} <- lex_expr__143(rest5),
           {:ok, t7, rest7} <- lex_expr__144(rest6) do
        {:ok, t0 <> t1 <> t2 <> t3 <> t4 <> t5 <> t6 <> t7, rest7}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__146(input) do
      Tokenizer.star_char(&lex_expr__147/1, input)
    end
    ...
    def lex_expr__160(input0) do
      with {:ok, t0, rest0} <- lex_expr__161(input0),
           {:ok, t1, rest1} <- lex_expr__162(rest0),
    ...
    def lex_expr__159(input) do
      Tokenizer.not_pred_char(&lex_expr__160/1, input)
    end
    ...
    def lex_expr__158(input0) do
      with {:ok, t0, rest0} <- lex_expr__159(input0), {:ok, t1, rest1} <- lex_expr__164(rest0) do
        {:ok, t0 <> t1, rest1}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__155(input) do
      Tokenizer.star_char(&lex_expr__156/1, input)
    end
    ...
    def lex_expr__165(input0) do
      with {:ok, t0, rest0} <- lex_expr__166(input0),
           {:ok, t1, rest1} <- lex_expr__167(rest0),
           {:ok, t2, rest2} <- lex_expr__168(rest1) do
        {:ok, t0 <> t1 <> t2, rest2}
      else
        :fail -> :fail
      end
    ...
    def lex_token__MULTILINE_STRING(input0) do
      with {:ok, t0, rest0} <- lex_expr__151(input0),
           {:ok, t1, rest1} <- lex_expr__155(rest0),
           {:ok, t2, rest2} <- lex_expr__165(rest1) do
        {:ok, t0 <> t1 <> t2, rest2}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__177(input0) do
      with {:ok, t0, rest0} <- lex_expr__178(input0),
           {:ok, t1, rest1} <- lex_expr__179(rest0),
    ...
    def lex_expr__185(input) do
      Tokenizer.plus_char(&lex_expr__186/1, input)
    end

    def lex_expr__182(input0) do
      with {:ok, t0, rest0} <- lex_expr__183(input0),
           {:ok, t1, rest1} <- lex_expr__184(rest0),
           {:ok, t2, rest2} <- lex_expr__185(rest1) do
        {:ok, t0 <> t1 <> t2, rest2}
      else
        :fail -> :fail
      end
    ...
    def lex_token__RADIX(input) do
      Tokenizer.first_char_match([&lex_expr__177/1, &lex_expr__182/1, &lex_expr__187/1], input)
    end

    def lex_expr__192(input) do
      case input do
        <<c::utf8, rest::binary>> ->
          if Tokenizer.in_ranges?(c, [{118, 118}, {86, 86}]) do
            {:ok, <<c::utf8>>, rest}
          else
            :fail
          end
    ...
    def lex_token__KW_VIA(input0) do
      with {:ok, t0, rest0} <- lex_expr__192(input0),
           {:ok, t1, rest1} <- lex_expr__193(rest0),
    ...
    def lex_expr__198(input0) do
      with {:ok, t0, rest0} <- lex_expr__199(input0), {:ok, t1, rest1} <- lex_expr__200(rest0) do
        {:ok, t0 <> t1, rest1}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__197(input) do
      Tokenizer.opt_char(&lex_expr__198/1, input)
    end
    ...
    def lex_expr__210(input) do
      Tokenizer.plus_char(&lex_expr__211/1, input)
    end
    ...
    def lex_expr__212(input) do
      Tokenizer.opt_char(&lex_expr__213/1, input)
    end
    ...
    def lex_token__BYTE_SIZE(input0) do
      with {:ok, t0, rest0} <- lex_expr__210(input0),
           {:ok, t1, rest1} <- lex_expr__212(rest0),
           {:ok, t2, rest2} <- lex_expr__217(rest1) do
        {:ok, t0 <> t1 <> t2, rest2}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__229(input) do
      Tokenizer.first_char_match([&lex_expr__230/1, &lex_expr__231/1], input)
    end

    def lex_token__TRIVIA(input) do
      Tokenizer.star_char(&lex_expr__229/1, input)
    end
    ...
    def lex_token__KW_HOPS(input0) do
      with {:ok, t0, rest0} <- lex_expr__232(input0),
           {:ok, t1, rest1} <- lex_expr__233(rest0),
           {:ok, t2, rest2} <- lex_expr__234(rest1),
           {:ok, t3, rest3} <- lex_expr__235(rest2) do
        {:ok, t0 <> t1 <> t2 <> t3, rest3}
      else
        :fail -> :fail
      end
    ...
    def lex_token__NOT_EQ(input0) do
      with {:ok, t0, rest0} <- lex_expr__236(input0),
           {:ok, t1, rest1} <- lex_expr__237(rest0),
           {:ok, t2, rest2} <- lex_expr__238(rest1),
    ...
    def lex_token__KW_DEEP(input0) do
      with {:ok, t0, rest0} <- lex_expr__240(input0),
           {:ok, t1, rest1} <- lex_expr__241(rest0),
           {:ok, t2, rest2} <- lex_expr__242(rest1),
           {:ok, t3, rest3} <- lex_expr__243(rest2) do
        {:ok, t0 <> t1 <> t2 <> t3, rest3}
      else
        :fail -> :fail
      end
    ...
    def lex_expr__249(input) do
      Tokenizer.first_char_match([&lex_expr__250/1, &lex_expr__251/1], input)
    end

    def lex_expr__248(input) do
      Tokenizer.star_char(&lex_expr__249/1, input)
    end

    def lex_token__PARAM(input0) do
      with {:ok, t0, rest0} <- lex_expr__244(input0),
    ...
    def lex_candidates(input, context) do
      [
        ...
        KW_DEEP: fn ->
          case lex_token__KW_DEEP(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_PARENT: fn ->
          case lex_token__KW_PARENT(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_SIBLINGS: fn ->
          case lex_token__KW_SIBLINGS(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_ANCESTORS: fn ->
          case lex_token__KW_ANCESTORS(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_VIA: fn ->
          case lex_token__KW_VIA(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_PATH: fn ->
          case lex_token__KW_PATH(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_SHORTEST: fn ->
          case lex_token__KW_SHORTEST(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_BACKWARD: fn ->
          case lex_token__KW_BACKWARD(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
        KW_HOPS: fn ->
          case lex_token__KW_HOPS(input) do
            {:ok, text, rest} -> {:ok, text, rest, nil}
            :fail -> :fail
          end
        end,
    ...
    def parse_expr__0(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__1(stream, pos, ref_stack, context),
           ...
           {:ok, pos4, ref4, cap3} <- parse_expr__5(stream, pos3, ref3, context) do
        {:ok, pos4, ref4,
    ...
    def parse_rule__body_list_tail(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__0/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__17(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__18(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__20(stream, pos1, ref1, context) do
    ...
    def parse_expr__16(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__17/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__10(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__11(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__16(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__26(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__27/4, &parse_expr__28/4], stream, pos, ref_stack, context)
    end

    def parse_expr__25(stream, pos, ref_stack, context) do
      case (&parse_expr__26/4).(stream, pos, ref_stack, context) do
    ...
    def parse_expr__40(stream, pos, ref_stack, context) do
      case (&parse_expr__41/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_expr__46(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__47(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__50(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__51(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__52(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__55(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__combinator_tail(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__36/4, &parse_expr__46/4, &parse_expr__51/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__62(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__63/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__call(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__56(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__59(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__60(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__62(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__64(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__66(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__69(stream, pos, ref_stack, context) do
      case parse_rule__parent_field(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, parent_field: {:rule, :parent_field, sub_captures}}
    ...
    def parse_expr__70(stream, pos, ref_stack, context) do
      case parse_rule__siblings_field(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, siblings_field: {:rule, :siblings_field, sub_captures}}
    ...
    def parse_expr__71(stream, pos, ref_stack, context) do
      case parse_rule__ancestors_field(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, ancestors_field: {:rule, :ancestors_field, sub_captures}}
    ...
    def parse_expr__68(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__69/4, &parse_expr__70/4, &parse_expr__71/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__73(stream, pos, ref_stack, context) do
      case parse_rule__via_item(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, via_item: {:rule, :via_item, sub_captures}}
    ...
    def parse_expr__74(stream, pos, ref_stack, context) do
      case parse_rule__path_item(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, path_item: {:rule, :path_item, sub_captures}}
    ...
    def parse_expr__72(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__73/4, &parse_expr__74/4], stream, pos, ref_stack, context)
    end

    def parse_rule__body_item_ep1(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__67/4, &parse_expr__68/4, &parse_expr__72/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__82(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__83(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__85(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__81(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__82/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__79(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__80(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__81(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__78(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__79/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__combined_select(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__75(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__78(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__100(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__101(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__104(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__type_field(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__109(stream, pos, ref_stack, context),
           ...
           {:ok, pos5, ref5, cap4} <- parse_expr__115(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__with_decl(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__116(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__119(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__120(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__122(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__123(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__125(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__135(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_PARENT) do
        {:ok, new_pos, text, nil} ->
          {:ok, new_pos, ref_stack, KW_PARENT: {:token, :KW_PARENT, text}}

        {:ok, new_pos, _text, capture} ->
          {:ok, new_pos, ref_stack, KW_PARENT: capture}
    ...
    def parse_expr__141(stream, pos, ref_stack, context) do
      case parse_rule__body_list(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, inner: {:rule, :body_list, sub_captures}}
    ...
    def parse_rule__parent_field(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__135(stream, pos, ref_stack, context),
           ...
           {:ok, pos5, ref5, cap4} <- parse_expr__141(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__142(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__144(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__159(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__160(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__163(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__156(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__157(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__159(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__155(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__156/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__fragment_decl(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__164(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__167(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__168(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__170(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__171(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__173(stream, pos6, ref6, context),
           {:ok, pos8, ref8, cap7} <- parse_expr__174(stream, pos7, ref7, context),
           {:ok, pos9, ref9, cap8} <- parse_expr__176(stream, pos8, ref8, context) do
        {:ok, pos9, ref9,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__window_call(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__177(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__180(stream, pos2, ref2, context),
    ...
    def parse_expr__194(stream, pos, ref_stack, context) do
      case (&parse_expr__195/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_rule__backward_clause(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_BACKWARD) do
        {:ok, new_pos, text, nil} ->
          {:ok, new_pos, ref_stack, KW_BACKWARD: {:token, :KW_BACKWARD, text}}

        {:ok, new_pos, _text, capture} ->
          {:ok, new_pos, ref_stack, KW_BACKWARD: capture}
    ...
    def parse_expr__205(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__206/4, &parse_expr__207/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__210(stream, pos, ref_stack, context) do
      Parser.star(&parse_expr__211/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__213(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__214/4, &parse_expr__215/4], stream, pos, ref_stack, context)
    end

    def parse_expr__212(stream, pos, ref_stack, context) do
      case (&parse_expr__213/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_expr__208(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__209(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__210(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__212(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__frame_bound(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__200/4, &parse_expr__208/4, &parse_expr__216/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_rule__group_by_cube(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__221(stream, pos, ref_stack, context),
           ...
           {:ok, pos6, ref6, cap5} <- parse_expr__228(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__230(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__path_item(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_PATH) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, KW_PATH: {:token, :KW_PATH, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, KW_PATH: capture}
        :fail -> :fail
      end
    end
    ...
    def parse_expr__241(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__242/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__when_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__250(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__253(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__254(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__256(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__257(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__259(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__body_item(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__260/4, &parse_expr__261/4, &parse_expr__262/4, &parse_expr__263/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__270(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__271(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__272(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__274(stream, pos2, ref2, context),
    ...
    def parse_expr__292(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__293/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__312(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__313/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__310(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__311(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__312(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__309(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__310/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__document(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__289(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__297(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__299(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__307(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__309(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__317(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__319(stream, pos6, ref6, context),
           {:ok, pos8, ref8, cap7} <- parse_expr__320(stream, pos7, ref7, context),
           {:ok, pos9, ref9, cap8} <- parse_expr__322(stream, pos8, ref8, context) do
        {:ok, pos9, ref9,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__332(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__333(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__336(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__field_body_item(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__324/4, &parse_expr__332/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__341(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__342(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__344(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__340(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__341/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__357(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__358/4, &parse_expr__359/4, &parse_expr__360/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_rule__list_type(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__361(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__364(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__365(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__367(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__frame_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__368(stream, pos, ref_stack, context),
           ...
           {:ok, pos5, ref5, cap4} <- parse_expr__374(stream, pos4, ref4, context),
           ...
           {:ok, pos9, ref9, cap8} <- parse_expr__380(stream, pos8, ref8, context) do
        {:ok, pos9, ref9,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__fragment_or_comment(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__430/4, &parse_expr__431/4], stream, pos, ref_stack, context)
    end

    def parse_expr__432(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_SIBLINGS) do
        {:ok, new_pos, text, nil} ->
          {:ok, new_pos, ref_stack, KW_SIBLINGS: {:token, :KW_SIBLINGS, text}}

        {:ok, new_pos, _text, capture} ->
          {:ok, new_pos, ref_stack, KW_SIBLINGS: capture}
    ...
    def parse_rule__siblings_field(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__432(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__433(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__435(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__436(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__438(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__439(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__441(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__446(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__447/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__450(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__451/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__over_spec(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__442(stream, pos, ref_stack, context),
           ...
           {:ok, pos5, ref5, cap4} <- parse_expr__450(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__457(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__458(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__461(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__462(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__463/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__456(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__457(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__462(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__path(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__452(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__455(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__474(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__475/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__487(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__488/4, stream, pos, ref_stack, context)
    end

    def parse_expr__486(stream, pos, ref_stack, context) do
      case (&parse_expr__487/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_rule__body_list(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__482(stream, pos, ref_stack, context),
           ...
           {:ok, pos4, ref4, cap3} <- parse_expr__486(stream, pos3, ref3, context) do
        {:ok, pos4, ref4,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(cap1, Parser.merge_captures(cap2, cap3))
    ...
    def parse_rule__goal_args(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__489(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__492(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__494(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__496(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__498(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__499/4, stream, pos, ref_stack, context)
    end

    def parse_expr__497(stream, pos, ref_stack, context) do
      case (&parse_expr__498/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_rule__in_lhs(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__503/4, &parse_expr__504/4, &parse_expr__505/4, &parse_expr__506/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__507(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_HOPS) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, KW_HOPS: {:token, :KW_HOPS, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, KW_HOPS: capture}
        :fail -> :fail
      end
    end
    ...
    def parse_expr__510(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :INTEGER) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, from: {:token, :INTEGER, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, from: capture}
        :fail -> :fail
      end
    end
    ...
    def parse_expr__516(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :INTEGER) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, to: {:token, :INTEGER, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, to: capture}
        :fail -> :fail
      end
    end

    def parse_rule__hops_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__507(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__510(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__511(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__513(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__514(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__516(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__523(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__524/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__partition_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__525(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__528(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__529(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__531(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__where_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__532(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__533(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__535(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__536(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_ANCESTORS) do
        {:ok, new_pos, text, nil} ->
          {:ok, new_pos, ref_stack, KW_ANCESTORS: {:token, :KW_ANCESTORS, text}}

        {:ok, new_pos, _text, capture} ->
          {:ok, new_pos, ref_stack, KW_ANCESTORS: capture}
    ...
    def parse_rule__ancestors_field(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__536(stream, pos, ref_stack, context),
           ...
           {:ok, pos5, ref5, cap4} <- parse_expr__542(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__543(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__545(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__551(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__552/4, &parse_expr__553/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__type_decl(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__554(stream, pos, ref_stack, context),
           ...
           {:ok, pos6, ref6, cap5} <- parse_expr__562(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__564(stream, pos6, ref6, context),
           {:ok, pos8, ref8, cap7} <- parse_expr__565(stream, pos7, ref7, context),
           {:ok, pos9, ref9, cap8} <- parse_expr__567(stream, pos8, ref8, context),
           {:ok, pos10, ref10, cap9} <- parse_expr__568(stream, pos9, ref9, context),
           {:ok, pos11, ref11, cap10} <- parse_expr__570(stream, pos10, ref10, context) do
        {:ok, pos11, ref11,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__group_by_rollup(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__571(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__574(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__575(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__577(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__578(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__580(stream, pos6, ref6, context) do
        {:ok, pos7, ref7,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__call_arg(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__581/4, &parse_expr__586/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__601(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__602(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__603(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__605(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__598(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__599(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__601(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__597(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__598/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__591(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__592(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__597(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__590(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__591/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__606(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_VIA) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, KW_VIA: {:token, :KW_VIA, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, KW_VIA: capture}
        :fail -> :fail
      end
    end
    ...
    def parse_expr__609(stream, pos, ref_stack, context) do
      case parse_rule__path(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, edge: {:rule, :path, sub_captures}}
    ...
    def parse_expr__613(stream, pos, ref_stack, context) do
      case parse_rule__shortest_clause(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, shortest_clause: {:rule, :shortest_clause, sub_captures}}
    ...
    def parse_expr__617(stream, pos, ref_stack, context) do
      case parse_rule__backward_clause(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, backward_clause: {:rule, :backward_clause, sub_captures}}
    ...
    def parse_expr__621(stream, pos, ref_stack, context) do
      case parse_rule__hops_clause(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, sub_captures} ->
          {:ok, new_pos, new_ref_stack, hops_clause: {:rule, :hops_clause, sub_captures}}
    ...
    def parse_expr__620(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__621/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__628(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__629/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__636(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__637/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__via_item(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__606(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__609(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__610(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__612(stream, pos4, ref4, context),
           {:ok, pos6, ref6, cap5} <- parse_expr__614(stream, pos5, ref5, context),
           {:ok, pos7, ref7, cap6} <- parse_expr__616(stream, pos6, ref6, context),
           {:ok, pos8, ref8, cap7} <- parse_expr__618(stream, pos7, ref7, context),
           {:ok, pos9, ref9, cap8} <- parse_expr__620(stream, pos8, ref8, context),
           {:ok, pos10, ref10, cap9} <- parse_expr__622(stream, pos9, ref9, context),
           {:ok, pos11, ref11, cap10} <- parse_expr__624(stream, pos10, ref10, context),
           {:ok, pos12, ref12, cap11} <- parse_expr__626(stream, pos11, ref11, context),
           {:ok, pos13, ref13, cap12} <- parse_expr__628(stream, pos12, ref12, context),
           {:ok, pos14, ref14, cap13} <- parse_expr__630(stream, pos13, ref13, context),
           {:ok, pos15, ref15, cap14} <- parse_expr__632(stream, pos14, ref14, context),
           {:ok, pos16, ref16, cap15} <- parse_expr__634(stream, pos15, ref15, context),
           {:ok, pos17, ref17, cap16} <- parse_expr__636(stream, pos16, ref16, context),
           {:ok, pos18, ref18, cap17} <- parse_expr__638(stream, pos17, ref17, context),
           {:ok, pos19, ref19, cap18} <- parse_expr__640(stream, pos18, ref18, context),
           {:ok, pos20, ref20, cap19} <- parse_expr__641(stream, pos19, ref19, context),
           {:ok, pos21, ref21, cap20} <- parse_expr__643(stream, pos20, ref20, context),
           {:ok, pos22, ref22, cap21} <- parse_expr__644(stream, pos21, ref21, context),
           {:ok, pos23, ref23, cap22} <- parse_expr__646(stream, pos22, ref22, context) do
        {:ok, pos23, ref23,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
                                           ...
                                           Parser.merge_captures(
                                             ...
                                             Parser.merge_captures(
                                               ...
                                               Parser.merge_captures(
                                                 ...
                                                 Parser.merge_captures(
                                                   cap20,
                                                   Parser.merge_captures(cap21, cap22)
                                                 )
                                               )
                                             )
                                           )
    ...
    def parse_expr__648(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_DEEP) do
        {:ok, new_pos, text, nil} -> {:ok, new_pos, ref_stack, KW_DEEP: {:token, :KW_DEEP, text}}
        {:ok, new_pos, _text, capture} -> {:ok, new_pos, ref_stack, KW_DEEP: capture}
        :fail -> :fail
      end
    end

    def parse_rule__select_ep1a(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__647/4, &parse_expr__648/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__with_or_comment(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__649/4, &parse_expr__650/4], stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__659(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__660(stream, pos, ref_stack, context),
           ...
           {:ok, pos4, ref4, cap3} <- parse_expr__664(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__666(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__667(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__668(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__671(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__672(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__674(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__675(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__676(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__679(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__680(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__682(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__683(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__684(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__687(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__688(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__690(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__shape_type(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__700(stream, pos, ref_stack, context),
           ...
           {:ok, pos4, ref4, cap3} <- parse_expr__704(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__706(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__724(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__725/4, &parse_expr__726/4], stream, pos, ref_stack, context)
    end

    def parse_expr__723(stream, pos, ref_stack, context) do
      case (&parse_expr__724/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
          {:ok, new_pos, new_ref_stack, Parser.merge_captures(inner_caps, op: {:text, text})}
    ...
    def parse_expr__737(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__738(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__740(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__736(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__737/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__734(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__735(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__736(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__expression(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__730(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__731(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__733(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__747(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__748/4, stream, pos, ref_stack, context)
    end

    def parse_expr__743(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__744(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__745(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__747(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__base_type(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__741/4, &parse_expr__742/4, &parse_expr__743/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_expr__760(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__761(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__763(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__759(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__760/4, 0, :infinity, stream, pos, ref_stack, context)
    end

    def parse_expr__753(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__754(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__759(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__type_kind(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__768(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__771(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__predicate_lhs(stream, pos, ref_stack, context) do
      Parser.try_alts(
        [&parse_expr__772/4, &parse_expr__773/4, &parse_expr__774/4],
        stream,
        pos,
        ref_stack,
        context
    ...
    def parse_rule__field_name(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__775/4, &parse_expr__776/4], stream, pos, ref_stack, context)
    end

    def parse_rule__shortest_clause(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :KW_SHORTEST) do
        {:ok, new_pos, text, nil} ->
          {:ok, new_pos, ref_stack, KW_SHORTEST: {:token, :KW_SHORTEST, text}}

        {:ok, new_pos, _text, capture} ->
          {:ok, new_pos, ref_stack, KW_SHORTEST: capture}
    ...
    def parse_expr__791(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__792(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__793(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__795(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__788(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__789(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__791(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__787(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__788/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__797(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__798/4, stream, pos, ref_stack, context)
    end

    def parse_expr__796(stream, pos, ref_stack, context) do
      case (&parse_expr__797/4).(stream, pos, ref_stack, context) do
        {:ok, new_pos, new_ref_stack, inner_caps} ->
          text = Parser.concat_text(stream, pos, new_pos)
    ...
    def parse_rule__type_operand(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__796(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__801(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__816(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__817(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__819(stream, pos1, ref1, context) do
        {:ok, pos2, ref2, Parser.merge_captures(cap0, cap1)}
      else
        _fail -> :fail
      end
    ...
    def parse_expr__815(stream, pos, ref_stack, context) do
      Parser.rep(&parse_expr__816/4, 0, :infinity, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__812(stream, pos, ref_stack, context) do
      Parser.opt(&parse_expr__813/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_expr__821(stream, pos, ref_stack, context) do
      Parser.star(&parse_expr__822/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__rational(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__820(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__821(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__823(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__824(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__826(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_rule__order_by_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__831(stream, pos, ref_stack, context),
           ...
           {:ok, pos3, ref3, cap2} <- parse_expr__834(stream, pos2, ref2, context),
           {:ok, pos4, ref4, cap3} <- parse_expr__835(stream, pos3, ref3, context),
           {:ok, pos5, ref5, cap4} <- parse_expr__837(stream, pos4, ref4, context) do
        {:ok, pos5, ref5,
         Parser.merge_captures(
           cap0,
           Parser.merge_captures(
    ...
    def parse_expr__840(stream, pos, ref_stack, _context) do
      case Parser.match_token(stream, pos, :TRIVIA) do
        {:ok, new_pos, _text, _capture} -> {:ok, new_pos, ref_stack, []}
        :fail -> :fail
      end
    ...
    def parse_expr__839(stream, pos, ref_stack, context) do
      Parser.star(&parse_expr__840/4, stream, pos, ref_stack, context)
    end
    ...
    def parse_rule__offset_clause(stream, pos, ref_stack, context) do
      with {:ok, pos1, ref1, cap0} <- parse_expr__838(stream, pos, ref_stack, context),
           {:ok, pos2, ref2, cap1} <- parse_expr__839(stream, pos1, ref1, context),
           {:ok, pos3, ref3, cap2} <- parse_expr__841(stream, pos2, ref2, context) do
        {:ok, pos3, ref3, Parser.merge_captures(cap0, Parser.merge_captures(cap1, cap2))}
      else
        _fail -> :fail
      end
    ...
    def parse_rule__type_or_comment(stream, pos, ref_stack, context) do
      Parser.try_alts([&parse_expr__842/4, &parse_expr__843/4], stream, pos, ref_stack, context)
    end
           ...
           {:ok, value, _context} <-
             Ichor.Actions.evaluate(
               ...
               %{
                 ...
                 list: MapSet.new([]),
                 backward_clause: MapSet.new([]),
                 frame_bound: MapSet.new([]),
                 group_by_cube: MapSet.new([]),
                 path_item: MapSet.new([]),
      ...
      if pos >= tuple_size(stream) do
        ...
        case parse_rule__document(stream, pos, [0], ctx) do
          {:ok, new_pos, _ref_stack, raw_captures} ->
            case Ichor.Actions.evaluate(:document, raw_captures, Scry.DocGraph.Actions, ctx, %{
                   body_list_tail: MapSet.new([]),
                   literal_list: MapSet.new([:tail, :COMMA]),
                   additive_tail: MapSet.new([]),
                   spread: MapSet.new([]),
